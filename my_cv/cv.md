# Pierre-Adrien LAIR

**Senior Lead Analytics & Applied AI Specialist**

Paris & Normandy, France

---

## Contact

- **Email:** lairpierreadrien@gmail.com
- **LinkedIn:** [linkedin.com/in/pierre-adrien-lair-55a6a850](https://www.linkedin.com/in/pierre-adrien-lair-55a6a850/)
- **X/Twitter:** [@pal_analytics](https://x.com/pal_analytics)

---

## Résumé Professionnel

Senior Lead avec 12 ans d'expérience en conseil digital et data, spécialisé dans l'accompagnement des grands comptes sur leurs enjeux de mesure, d'analytics et d'adoption de l'IA. Je dirige la R&D IA appliquée aux métiers analytics au sein d'une practice de 150 consultants, en développant des agents autonomes et des solutions d'automatisation basées sur les LLMs.

Mon moteur : permettre aux équipes et aux clients de travailler mieux demain qu'aujourd'hui, en concevant des architectures techniques qui allient innovation et pragmatisme business.

Convaincu que l'IA doit être déployée de manière responsable et bénéfique, je suis un promoteur actif des LLMs (Claude, Gemini, GPT) et du vibe coding au sein de mon organisation.

**Philosophie de carriere :** Ma progression de consultant a senior lead suit l'evolution naturelle en cabinet de conseil, mais resulte aussi d'opportunites que je me suis creees en cherchant toujours a faire au mieux. J'associe resultats techniques, expertise pointue, fidelisation client et developpement business - c'est cette combinaison qui m'a permis d'evoluer.

**Fiertes professionnelles :**
- Avoir aide des equipes a grandir et tente d'inculquer mes valeurs : "work proof" (prouver par le travail), esprit "hacking" (trouver des solutions creatives), et curiosite permanente
- Les projets IA/agents sont encore a l'etat de prototypes prometteurs - l'objectif est de demontrer le potentiel, pas de survendre
- Formation de 200+ consultants et clients transmission des methodologies

**Apprentissage d'un echec :** Pour un grand voyagiste, j'ai cree des agents autonomes de QA/recette analytics. J'ai sous-estime la mise en production (erreurs inattendues), la gestion des contextes longs, et globalement le vrai travail de dev necessaire pour assurer fiabilite et robustesse. Lecon : l'innovation IA doit etre ancree dans la realite metier, et le passage du prototype a la production demande une rigueur d'ingenierie que je continue d'apprendre.

**ADN productiviste :** Ma transition vers l'IA est naturelle car je suis fondamentalement oriente productivite et j'adore la promesse des agents autonomes. Anecdote fondatrice : j'ai demarre chez Converteo en stage, et mon maitre de stage me demandait d'utiliser Excel SANS souris - ca m'a marque et forge mon approche "raccourcis, automatisation, efficacite".

**Pourquoi Converteo :** Je suis TOUJOURS chez Converteo (pas parti). Belle progression de carriere + culture humaine dans un cabinet de conseil devenu leader sur les sujets web & app analytics. Opportunites d'evolution constantes et possibilite de me reinventer (analytics → IA). J'y ai demarre en stage et j'y suis encore Senior Lead aujourd'hui.

---

## Expériences Professionnelles

### Converteo — Senior Lead Analytics & AI
**Janvier 2025 - Présent** | Paris, France (Hybride)

Converteo est un cabinet de conseil leader en France, spécialiste du Digital et de la Data (450 collaborateurs).

**Responsabilités :**
- **Direction de la vision Analytics & Conversion** au sein de la practice (150+ personnes)
- **Définition et évolution des offres** : collecte de données, analyses & insights, application de l'IA aux métiers analytics
- **Management des partenariats stratégiques** : Google, Meta, AB Tasty, Kameleoon, Didomi, CommandersAct, Amplitude, Piano, Datama
- **Innovation & R&D IA** : responsable de l'exploration et du prototypage de solutions IA génératives appliquées aux métiers
- **Thought Leadership** : webinars, publications, représentation externe
- **Knowledge Management** : onboarding, standardisation des méthodologies, formation interne (200+ consultants et clients formés)

**Projets AI/LLM réalisés :**

| Projet | Technologie | Cas d'usage business |
|--------|-------------|---------------------|
| Agents de Tracking Management | Google ADK (Agent Dev Kit), Gemini API | Automatisation des tâches de tag management : déploiement de variables sur multiples tags GTM en une commande |
| Automatisation QA Analytics | Browserbase, Stagehand, Vercel AI SDK | Navigation autonome pour récupérer dataLayer & hits analytics à chaque étape du parcours utilisateur, automatisation de la recette |
| Personas Synthétiques UX | Browser Use, Steel Dev, Gemini API | Évaluation automatisée de l'UX d'un site et benchmark concurrentiel via agents de navigation |
| Outils internes productivité | Claude API (Anthropic) | Système d'assistance personnel pour les tâches quotidiennes, intégré comme OS de productivité |
| Promotion Claude Code | Claude Code CLI | Évangélisation interne et formation des équipes aux outils Anthropic |

**Clients accompagnés :**
Club Med, Fédération Française de Football (FFF), Sephora, Accor, Engie, FDJ, TotalEnergies, Hermès, L'Oréal, LVMH, Samsung, Yves Rocher

---

### Converteo — Lead Analytics
**Novembre 2019 - Décembre 2024** (5 ans 2 mois) | Paris, France

*Contexte : Retour a Paris & Normandie, opportunite de prendre des responsabilites elargies dans un cabinet de plus grande envergure (450 personnes) et de lancer la R&D IA.*

**Responsabilités :**
- Garant de la méthodologie et du delivery de la practice Analytics & Conversion
- Intervention experte sur les projets Analytics complexes
- Organisation et diffusion de la connaissance et de l'innovation
- R&D sur les sujets d'IA Générative (depuis 2022 avec GPT-3/Davinci)
- Formation interne des consultants
- Relations Partenaires et éditeurs

**Réalisations clés :**
- Structuration des méthodologies de tracking et d'analytics pour l'ensemble de la practice
- Lancement de l'initiative R&D IA générative appliquée aux métiers analytics
- Formation de 200+ consultants et clients sur les sujets tracking & analytics

---

### Empirik — Consultant Senior / Head of Analytics
**Mai 2017 - Novembre 2019** (2 ans 7 mois) | Lyon, France

*Contexte : Demenagement a Lyon pour raisons personnelles, opportunite de me challenger dans une structure plus petite et de prendre la responsabilite d'une offre complete.*

Empirik est une agence webmarketing spécialisée dans l'exploitation intelligente des données.

**Responsabilités :**
- **Responsable de l'offre Analytics** : accompagnement clients sur toute la chaîne de valeur data online
- **Conseil en choix d'outils** : Analytics, CRM, UX, A/B Testing, DMP, Marketing Automation, Data Visualisation
- **Tag Management** : plans de marquage, implémentation GTM, TagCommander, Adobe Tag Manager
- **Analytics** : déploiement et analyses (Adobe Analytics, AT Internet, Firebase, Google Analytics, Matomo, Mixpanel)
- **Data Engineering** : architecture données marketing dans Google Cloud / BigQuery, restitution Data Studio, BIME, Tableau
- **CRO & UX** : activation A/B Test (Google Optimize, AB Tasty, Kameleoon), outils UX (ContentSquare, Hotjar, Crazy Egg)
- **Attribution & Media** : MixCommander, Mazeberry
- **Formation** : Google Analytics, Google Tag Manager, Data Studio, Adobe Ad Hoc Analysis

---

### Converteo — Senior Consultant Digital & Data
**Février 2014 - Avril 2017** (3 ans 3 mois) | Paris, France

**Missions :**
- **AMOA Projets Digitaux** : définition et expression des besoins, évaluation prestataires, recette (Redmine, Jira)
- **Audits e-business & webanalytics** : analyses statistiques (GA, BigQuery, Adobe Analytics, Coremetrics), audits ergonomiques (Crazy Egg), audits quali/quanti
- **Recommandations stratégiques** : acquisition, segmentation, parcours cross-devices, marketing automation (Marketo)
- **Conception d'outils de suivi** : scorecards, reportings automatisés, tableaux de bord (Data Studio, Supermetrics)
- **Implémentation tracking** : plans de taggage, intégration TMS
- **Formation** : ateliers Google Analytics

**Exemples d'analyses :**
- Personnalisation parcours cross-canaux et cross-device
- Analyse performances media et mix-marketing multitouch
- Évaluation processus de marketing automation

---

### Kurt Salmon (now Accenture Strategy) — Communication Assistant (Stage)
**Mai 2012 - Décembre 2012** (8 mois) | Neuilly-sur-Seine, France

- Lancement de site web
- Relations presse
- Préparation media training
- Création supports promotionnels (Photoshop, Illustrator, InDesign)
- Lancement compte Twitter corporate
- Support communication interne & marketing

---

### Max Milo Editions — Attaché de Presse (Stage)
**Mai 2011 - Août 2011** (4 mois) | Paris, France

- Relations presse pour maison d'édition
- Support communication

---

## Compétences Techniques

### IA & LLM
| Catégorie | Technologies |
|-----------|--------------|
| LLM APIs | Claude API (Anthropic), Gemini API (Google), OpenAI API |
| Agent Frameworks | Google ADK (Agent Dev Kit), Claude Code |
| Browser Automation AI | Browserbase, Stagehand, Browser Use, Steel Dev |
| AI Development | Vercel AI SDK, Lovable |
| Prompt Engineering | Prompting avancé, évaluation de modèles |

### Analytics & Data
| Catégorie | Technologies |
|-----------|--------------|
| Web Analytics | Google Analytics (GA4), Adobe Analytics, AT Internet, Mixpanel, Amplitude, Firebase, Matomo |
| Tag Management | Google Tag Manager, TagCommander (CommandersAct), Adobe Tag Manager |
| Data Warehouse | Google BigQuery, Google Cloud Platform |
| Data Visualization | Looker Studio (Data Studio), Tableau, BIME |
| A/B Testing & CRO | AB Tasty, Kameleoon, Google Optimize |
| UX Analytics | ContentSquare, Hotjar, Crazy Egg |
| Attribution | Eulerian, MixCommander, Mazeberry |
| Consent Management | Didomi |

### Développement & Outils
| Catégorie | Technologies |
|-----------|--------------|
| Langages | Python, R, SQL, JavaScript |
| Cloud | Google Cloud Platform, BigQuery |
| Automatisation | APIs, Supermetrics, scripts d'automatisation |
| Gestion de projet | Jira, Redmine |
| Design | Photoshop, Illustrator, InDesign, Balsamiq |

---

## Certifications

| Certification | Organisme | Date |
|--------------|-----------|------|
| AppsFlyer Essentials | AppsFlyer | Juin 2025 |
| Kameleoon A/B Testing | Kameleoon | Février 2025 |
| Kameleoon Gold Partner | Kameleoon | Février 2025 |
| Amplitude Delivery Partner | Amplitude | Janvier 2025 |
| AB Tasty Product Recos & Merchandising | AB Tasty | Novembre 2024 |
| AB Tasty User Expert | AB Tasty | Septembre 2020 |
| Google Tag Manager Certified Partner | Google | Mars 2017 |
| Eulerian DDP | Eulerian | Mai 2016 |
| TagCommander | CommandersAct | Août 2015 |
| Google Analytics Certification | Google | Février 2014 |

---

## Formation

| Diplôme | Établissement | Année |
|---------|---------------|-------|
| Master Marketing, Business & Management | ICN Business School, Nancy | 2010-2014 |
| Exchange Program - Management Information Systems | Université de Sherbrooke, Canada | 2013 |
| Classe Préparatoire ECS | Lycée Carnot, Paris | 2007-2009 |
| Baccalauréat S Mathématiques (Mention Très Bien) | Lycée Jean-Jacques Rousseau, Montmorency | 2007 |

**Activités associatives :**
- Atom'ik : programme Cordées de la Réussite - ouverture culturelle et sociale aux jeunes

---

## Publications & Thought Leadership

### Articles

| Titre | Éditeur | Date |
|-------|---------|------|
| [Gutenberg, LLM, Claude Code ou le moment "presse à imprimer"](https://www.linkedin.com/pulse/gutenberg-llm-claude-code-ou-le-moment-presse-%C3%A0-imprimer-lair-jifhe/) | LinkedIn | 2024 |
| [Lookup : exploiter le potentiel de l'interface GA4 avec les collections](https://converteo.com/blog/lookup-exploiter-le-potentiel-de-linterface-ga4-avec-les-collections/) | Converteo | 2023 |
| [Comment débugger Google Analytics pour son site ?](https://www.empirik.fr/) | Empirik | Mai 2018 |
| [Google Analytics se met en conformité RGPD](https://www.empirik.fr/) | Empirik | Avril 2018 |
| [Comprendre et suivre le consommateur cross-canal](https://converteo.com/) | Converteo | Mars 2015 |

### Webinars & Interventions

| Sujet | Organisateur |
|-------|--------------|
| [Privacy : limiter les impacts data marketing & optimiser les taux de consentement](https://converteo.com/ressources/webinar-privacy-comment-limiter-les-impacts-data-marketing-optimiser-ses-taux-de-consentement/) | Converteo |
| [IA & Stratégie : optimisation de l'expérience client](https://converteo.com/ressources/webinar-ia-strategie-optimisation-experience-client/) | Converteo |
| [Consentement, cookies et traceurs : votre site web est-il en conformité ?](https://converteo.com/ressources/webinar-consentement-cookies-et-traceurs-votre-site-web-est-il-en-conformite/) | Converteo |

### Travaux Académiques

- **Ecosystem of mobile contactless payment in France** — Mémoire de Master, ICN Business School

---

## Langues

| Langue | Niveau |
|--------|--------|
| Français | Natif |
| Anglais | Courant (TOEIC Gold) — Capacité à mener des présentations techniques et conversations business avec des dirigeants |

---

## Clients de Référence

**Luxe & Beauté :** Hermès, LVMH, L'Oréal, Sephora, Yves Rocher

**Énergie :** TotalEnergies, Engie

**Hôtellerie & Tourisme :** Club Med, Accor

**Sport & Entertainment :** Fédération Française de Football (FFF), FDJ

**Tech & Retail :** Samsung

---

## Centres d'intérêt & Valeurs

- **IA Responsable :** Convaincu que les systèmes d'IA doivent être déployés de manière sûre et bénéfique pour les utilisateurs et la société
- **Transmission :** Passion pour la formation, le mentorat et l'accompagnement des équipes vers l'excellence
- **Innovation pragmatique :** Transformer les avancées technologiques en valeur business concrète pour les entreprises

---

## Pourquoi je veux rejoindre Anthropic

Je suis convaincu que nous vivons un moment charnière dans l'histoire de la technologie, comparable à l'invention de l'imprimerie. Les LLMs transforment fondamentalement notre façon de travailler et de créer de la valeur.

Après 3 ans à explorer et déployer des solutions IA génératives pour mes clients, et en tant qu'utilisateur quotidien et promoteur de Claude, je souhaite contribuer directement à l'adoption responsable de ces technologies au sein des grandes entreprises.

Mon expérience unique combine :
- **12 ans d'accompagnement technique de grands comptes** sur des cycles d'achat complexes
- **Une expertise hands-on en développement d'agents et solutions LLM**
- **Une capacité éprouvée à traduire des concepts techniques en valeur business** pour des audiences variées (C-level, équipes IT, développeurs)

Je veux aider les entreprises à comprendre et exploiter le potentiel de Claude, tout en contribuant à la mission d'Anthropic : créer une IA fiable, interprétable et bénéfique.

---

*Dernière mise à jour : Janvier 2025*
